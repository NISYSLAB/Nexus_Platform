...
######## Env
## us.gcr.io/cloudypipelines-com/dicom2nifti_python:1.1
IMAGE_NAME=dicom2nifti_python
IMAGE_TAG=1.1
...
uid=$((1 + $RANDOM % 5000))
containerName="${IMAGE_NAME}_${uid}"
MOUNT=$PWD/"${IMAGE_NAME}_${uid}"
...
function run_docker() {
  local cmdArgs="${MOUNT}/run_model_predict.sh ${MOUNT}/trained_model.tar.gz ${MOUNT}/test_data.tar.gz ${version} ${trainedModelOutputs} ${savedResults}"
  docker run --rm -t \
        -v ${MOUNT}:${MOUNT} \
        --name ${containerName}  \
        -e DISK_MOUNTS=${MOUNT} \
