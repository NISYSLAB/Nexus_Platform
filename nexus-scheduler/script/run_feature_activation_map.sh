...
execScript=$HOME/workspace/Nexus_Platform/workflows/fMRI_Biomarker/script/run_model_feature_activation_map.sh

uid=$((1 + $RANDOM % 5000))
containerName="feature_activation_map_fmri_biomarker_${uid}"
MOUNT=$PWD/"mount_feature_activation_map_${uid}"

version=1
trainedModelOutputs="trained_model"
savedResults="activation_saved_results"

TASK_CALL_NAME=modelFeatureActivationMap
WORKFLOW_ID=$(uuidgen)
COPY_RESULTS=Y
...
function pre_run() {
    ...
    mkdir -p ${MOUNT}
    print_info "cp ${execScript} ${MOUNT}/run_model_feature_activation_map.sh"
    cp ${execScript} ${MOUNT}/run_model_feature_activation_map.sh
...
function run_docker() {
  local cmdArgs="${MOUNT}/run_model_feature_activation_map.sh ${MOUNT}/trained_model.tar.gz ${MOUNT}/test_data.tar.gz ${version} ${trainedModelOutputs} ${savedResults}"
  docker run --rm -t \
        -v ${MOUNT}:${MOUNT} \
        --name ${containerName}  \
        -e DISK_MOUNTS=${MOUNT} \
