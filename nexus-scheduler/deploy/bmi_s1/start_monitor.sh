...
function kill_monitor_instance() {
    echo "ps -eaf | grep rtcl-directory-monitor | grep java | grep jar"
    ps -eaf | grep rtcl-directory-monitor | grep java | grep jar
    process_id=$( ps -eaf | grep rtcl-directory-monitor | grep java | grep jar | awk '{print $2}' )
