workflow wf_fmri_biomarker {
    ...
    call modelTraining {
        input: trainDataIn = trainData, testDataIn = testData, usageMonitor = monitorScript, dockerInUse = dockerRef
    }

    call modelFeatureActivationMap {
        input: trainedModelData = modelTraining.out, testDataIn = testData, usageMonitor = monitorScript, dockerInUse = dockerRef
    }
...
task modelTraining {
    ...
    command {
        ...
        echo "./run_model_training.sh ${trainDataIn} ${testDataIn} ${version} ${outputs} ${savedResults}"
        ./run_model_training.sh ${trainDataIn} ${testDataIn} ${version} ${outputs} ${savedResults}
...
task modelFeatureActivationMap {
    ...
    String version
    File execScript
    String outputs = "feature_act_map_trained_model"
    String savedResults = "map_saved_results"
    File usageMonitor
    String dockerInUse
    command {
        mv ${usageMonitor} ./usage_monitor.sh
        ...
        echo "./run_model_feature_activation_map.sh ${trainedModelData} ${testDataIn} ${version} ${outputs} ${savedResults}"
        ./run_model_feature_activation_map.sh ${trainedModelData} ${testDataIn} ${version} ${outputs} ${savedResults}
