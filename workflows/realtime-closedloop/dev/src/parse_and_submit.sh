...
function printConfig() {
   ...
   printInfo "RTCP_MONITORING_IMAGE_DIR=$RTCP_MONITORING_IMAGE_DIR"
...
## We wait at least 1 second to make sure the file transfer is complete
...
function waitFileReady() {
    local dir=$1
    local fileWildcard=$2
    local timer=0
    echo "check if $fileWildcard is available"
    file=$(find ${dir} -name ${fileWildcard} -print -quit)
    while [[ ! -f "$file" ]]
    do
        ...
        then
          echo "$fileWildcard not available after waiting $WAITING_TIME_IN_SECONDS seconds, give up"
          return
        fi
    done
...
function submit2Pipeline() {
    ...
    mkdir -p ${tmpName}
    local x=${ceilStart}
    ## do the check outside of the loop only for the last file
    local dicomfileWildcardend=${RTCP_IMAGE_NAME_PART1}_${RTCP_IMAGE_NAME_PART2}_$(padding ${ceilEnd})_*.dcm
    echo "Waiting for the last file:${dicomfileWildcard}"
    waitFileReady ${RTCP_MONITORING_IMAGE_DIR} ${dicomfileWildcardend}
    while [ $x -le ${ceilEnd} ]
    do
      local dicomfile=${RTCP_IMAGE_NAME_PART1}_${RTCP_IMAGE_NAME_PART2}_$(padding ${x}).dcm
      ## After the scanner update the filename is a bit different now
      local dicomfileWildcard=${RTCP_IMAGE_NAME_PART1}_${RTCP_IMAGE_NAME_PART2}_$(padding ${x})_*.dcm
      echo "Copy dicom file:${dicomfileWildcard}"
      ## waitFileReady ${RTCP_MONITORING_IMAGE_DIR} ${dicomfileWildcard}
      cp ${RTCP_MONITORING_IMAGE_DIR}/${dicomfileWildcard} ${tmpName}/${dicomfile}
      x=$(( $x + 1 ))
    done
    ...
    time eval ${cmd_line}
    echo "Pipeline script run complete."
}
...
source ${RTCP_RUNTIME_DEFAULT_SETTINGS}
source ${RTCP_RUNTIME_USER_SETTINGS}
## Image incoming folder, processing the wildcard
RTCP_MONITORING_IMAGE_DIR=$(find "$MONITORING_IMAGE_DIR_ROOT" -type d -name "$RTCP_MONITORING_IMAGE_DIR")
echo "User configurations"
env |grep "RTCP_"
time execMain "${file}"
echo "Workflow loop complete."
