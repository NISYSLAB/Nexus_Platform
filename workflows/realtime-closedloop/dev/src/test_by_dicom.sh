...
function pre_process() {
    ...
    local host_dir=${HOST_MOUNT}/${TASK_CALL_NAME}/${WORKFLOW_ID}
    mkdir -p ${host_dir}
    ...
    cp /labs/mahmoudilab/synergy_remote_data1/emory_siemens_scanner_in_dir/image/wRewSig_Z_map.nii ${host_dir}/subject_mask.nii
...
function main_run() {
    ...
    docker exec ${CONTAINER_NAME} ${cmdArgs} 2>&1 | tee -a ${host_dir}/process_$( date +'%Y-%m-%d' ).log
