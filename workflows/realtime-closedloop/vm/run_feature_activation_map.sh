...
execScript=${actmap_exec_script}

uid=$((1 + $RANDOM % 5000))
containerName="feature_activation_map_fmri_biomarker_${uid}"
MOUNT=$PWD/"mount_activation_map_${uid}"
...
function pre_run() {
    ...
    mkdir -p ${MOUNT}
    print_info "cp ${execScript} ${MOUNT}/actmap_exec.sh"
    cp ${execScript} ${MOUNT}/actmap_exec.sh
...
function run_docker() {
  local cmdArgs="${MOUNT}/actmap_exec.sh ${MOUNT}/trained_model.tar.gz ${MOUNT}/test_data.tar.gz ${version} ${trainedModelOutputs} ${savedResults}"
  docker run --rm -t \
        -v ${MOUNT}:${MOUNT} \
        --name ${containerName}  \
	-e DISK_MOUNTS=${MOUNT} \
