...
def BayesOpt(filename, obj_filename):
    rows=[]  # optimizer is always called after the task run at least 1 trial
    ...
    current_obj = obj[0]
    # rows[len(rows)-1][2] = float(obj[current_obj])
    if len(rows)< N_burn_in: # initial random samples
        ...
        write_output_row(rows,q1,q2,current_obj,flag_end)
        ...
        # the current objective is the result generated by the last set of parameters sent to task computer
        Y_all = np.append(Y_all[1:], float(current_obj))
        ...
        if result.is_ok:
            ...
            write_output_row(rows,query_points[-1][0].numpy(), query_points[-1][1].numpy(),current_obj,flag_end)
        else:
            ...
            write_output_row(rows,query[-1][0], query[-1][1],current_obj,flag_end)




    # optimizer is always called after the task run at least 1 trial, this will be handled in burnout easily
...
def write_output_row(rows,q1,q2,obj,flag_end):
    new_row = [str(q1), str(q2), str(obj), str(flag_end)]
    rows.append(new_row)

    with open(filename, 'w') as csvfile:    # filename is global in this file, this is bad programming practice probably
        # creating a csv writer object
        csvwriter = csv.writer(csvfile, dialect='unix', quoting=csv.QUOTE_MINIMAL)  # specifying the format helps for testing without linux environment
        # writing the data rows
        csvwriter.writerows(rows)
        print('Optimizer generated new output in line %d' % (len(rows)))
